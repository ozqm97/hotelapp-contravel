...
export const establecerToken = (token) => {
    console.log("Estableciendo token en cookies:", token);
    if (!token) return;

    try {
        const parts = token.split(".");
        if(parts.length === 3) {
            Cookies.set(COOKIE_HEAD, parts[0], { secure: true, sameSite: 'strict' });
            Cookies.set(COOKIE_TOKEN, parts[1], { secure: true, sameSite: 'strict' });
            Cookies.set(COOKIE_KEY, parts[2], { secure: true, sameSite: 'strict' });
